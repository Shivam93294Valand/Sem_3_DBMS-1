...
SELECT ACTNO,CNAME,AMOUNT FROM DEPOSIT WHERE BNAME IN('AJNI','KAROLBAGH','M.G.ROAD') AND ACTNO<'105'; 
...
SELECT TOP 25 PERCENT CNAME FROM DEPOSIT WHERE AMOUNT>'0';
...
SELECT  *, AMOUNT * 1.1 AS EXT_AMOUNT FROM BORROW;

--7. Retrieve all odd/even value loan number from Borrow table.
SELECT LOANNO,CNAME,BNAME,AMOUNT FROM BORROW WHERE LOANNO % 2 <> 0;
SELECT LOANNO,CNAME,BNAME,AMOUNT FROM BORROW WHERE LOANNO % 2 = 0;
